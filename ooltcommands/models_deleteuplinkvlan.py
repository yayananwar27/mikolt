...
class OltCommanddeleteUplinkVlanModel(db.Model):
    __tablename__ = 'oltcommanddeleteuplinkvlan'
...
def insert_initial_data(*args, **kwargs):
    data_1 = OltCommanddeleteUplinkVlanModel(
        id_software=1, 
        script_python='''
...
output = delete_vlan_telnet_to_olt(host, username, password, port, interface, vlan_tag)
        '''
    )
    db.session.add(data_1)
    db.session.commit()

event.listen(OltCommanddeleteUplinkVlanModel.__table__, 'after_create', insert_initial_data)