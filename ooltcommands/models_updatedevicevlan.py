...
class OltCommandupdateDeviceVlanModel(db.Model):
    __tablename__ = 'oltcommandupdatedevicevlan'
...
def insert_initial_data(*args, **kwargs):
    data_1 = OltCommandupdateDeviceVlanModel(
        id_software=1, 
        script_python='''
def update_vlan_telnet_to_olt(host, username, password, port, data):
    import pexpect
    import re
    try:
...
output = update_vlan_telnet_to_olt(host, username, password, port, data)
        '''
    )
    db.session.add(data_1)
    db.session.commit()

event.listen(OltCommandupdateDeviceVlanModel.__table__, 'after_create', insert_initial_data)